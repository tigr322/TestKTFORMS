...
BEGIN
    ...
    SELECT
        ...
        SUM(CASE WHEN v.Code = 'POS' THEN ISNULL(d.CountVol,0) ELSE 0 END) AS [КОЛИЧЕСТВО ПОСЕЩЕНИЙ],
        SUM(CASE WHEN v.Code = 'OBR' THEN ISNULL(d.CountVol,0) ELSE 0 END) AS [КОЛИЧЕСТВО ОБРАЩЕНИЙ],
    ...
    LEFT JOIN rfcVolume           v ON v.ID = d.VolumeRef
    WHERE m.YearCode  = @pYear
      AND m.MonthCode = @pMonth
